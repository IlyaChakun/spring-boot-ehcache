...
public class Controller {
    ...
    @Autowired
    private EventBus<TestApmEvent> eventBus;
    ...
    public ResponseEntity<String> subscribe(@RequestBody EventBusSubscriberSettings settings) throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<List<TestApmEvent>> subscriber = eventBus.subscribe(
        ...
        List<TestApmEvent> events = subscriber.get();
    ...
    public ResponseEntity<String> publishEvent(@RequestBody EventBusEventPublishSettings settings) {
        ...
        eventBus.publishEvent(settings.getContextKey(), new TestApmEvent(settings.getEventKey()));
    ...
    private void waitForRequestEvents(ExecutorService executor, String contextKey, CompletableFuture<List<TestApmEvent>> request) {
        executor.submit(() -> {
            List<TestApmEvent> receivedRequestEvents = null;
            try {
    ...
    private void waitForPaymentEvents(ExecutorService executor, String contextKey, CompletableFuture<List<TestApmEvent>> payment, String authorizationConfirmed) {
        executor.submit(() -> {
            try {

                List<TestApmEvent> receivedPaymentEvents = payment.get();
                ...
                eventBus.publishEvent(contextKey, new TestApmEvent(authorizationConfirmed));
    ...
    private void waitForRegistrationEvents(ExecutorService executor, String contextKey,
                                           CompletableFuture<List<TestApmEvent>> registration, String registrationConfirmed) {
        executor.submit(() -> {
            try {
                List<TestApmEvent> receivedRegistrationEvents = registration.get();
                ...
                eventBus.publishEvent(contextKey, new TestApmEvent(registrationConfirmed));
    ...
    public void testFlow() {
        ...
        CompletableFuture<List<TestApmEvent>> request = eventBus.subscribe(
                contextKey,
        ...
        CompletableFuture<List<TestApmEvent>> registration = eventBus.subscribe(
                contextKey,
                events -> {
        ...
        CompletableFuture<List<TestApmEvent>> payment = eventBus.subscribe(
                contextKey,
                events -> {
                    Set<String> keys = events.stream()
                            .map(TestApmEvent.class::cast)
        ...
        eventBus.publishEvent(contextKey, new TestApmEvent(authorizationSuccessful));

        System.out.println("Publishing event..: " + registrationSuccessful);
        eventBus.publishEvent(contextKey, new TestApmEvent(registrationSuccessful));
